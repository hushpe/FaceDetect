...
std::vector<FaceInfo> UltraFace::detect(cv::Mat &img, std::string dst) {
  std::vector<FaceInfo> face_list = {};
       ...
       for(auto &ele: face_list) {
           cv::rectangle(img,cv::Rect(ele.x1,ele.y1,ele.x2 - ele.x1 +1,ele.y2 -ele.y1 +1),cv::Scalar(0,255,255),1,1,0);
       }
